...
class Reccurrent_layer(nn.Module):
    def __init__(self, input_dim, out_dim, hidden_units, blocks):
        super().__init__()

        self.fcin = nn.Linear(input_dim + out_dim, hidden_units)
        self.blocks = nn.ModuleList(
            [nn.Linear(hidden_units, hidden_units) for i in range(blocks)])

        self.fcout = nn.Linear(hidden_units, out_dim)

    def rnn_cell(self, x, h, c):
        state = torch.cat((x, h), dim=1)
        hidden_state = F.relu(self.fcin(state))
        if hidden_state.abs().max()> 1000:
            pdb.set_trace()
        cell_state = torch.tanh(torch.mul(c, hidden_state))

        for fc in self.blocks:
            hidden_state = F.relu(fc(hidden_state))
            if hidden_state.abs().max()> 1000:
                pdb.set_trace()
        
        cell_state = cell_state + torch.tanh(hidden_state)

        hidden_state = torch.mul(cell_state, hidden_state)
        if hidden_state.abs().max()> 1000:
            pdb.set_trace()

        return self.fcout(hidden_state), cell_state
...
class Encoder(Reccurrent_layer):
    def __init__(self, action_dim, z_dim, hidden_units, blocks):
        super().__init__(action_dim, hidden_units, hidden_units, blocks)
        
        self.mu = nn.Linear(hidden_units, z_dim)
        self.log_std = nn.Linear(hidden_units, z_dim)
        self.z_dim = z_dim
        self.hidden_units = hidden_units

    def forward(self, skill):
        h = skill.new(skill.shape[0], self.hidden_units)
        c = skill.new(skill.shape[0], self.hidden_units)
        
        for i in range(skill.shape[1]):
            h, c = self.rnn_cell(skill[:, i, :], h, c)
...
class Decoder(Reccurrent_layer):
    def __init__(self, action_dim, z_dim, skill_length, hidden_units, blocks):
        super().__init__(z_dim, action_dim, hidden_units, blocks)
    ...
    def forward(self, z):
        h = z.new(z.shape[0], self.action_dim)
        c = z.new(z.shape[0], self.hidden_units)
        x_hat = []

        for i in range(self.skill_length):
            h, c = self.rnn_cell(z, h, c)
            x_hat.append(h)

        x_hat = torch.stack(x_hat).swapaxes(0, 1)
        return x_hat
