...
class Sampler(hyper_params):
    def __init__(self, policy, decoder, actiondecoder, args):
        ...
        self.variables = ['state', 'next_state', 'z', 'next_z', 'actions',
                          ...
                          'reward_ctrl', 'states', 'reward']

        self.test_tasks = np.sort(np.random.uniform(
            0.0, 2.0, (self.meta_batch_size,)))
    ...
    def skill_execution(self, actions, env_ids):
        ...
        for i in range(actions.shape[1]):
            state, rew, done, info = self.env.step(actions[:, i, :],
                                                   env_id=env_ids)
            states.append(state)
    ...
    def skill_step(self, params, state, data, tasks):
        ...
        with torch.no_grad():
            ...
            actions = functional_call(
                self.ActionDecoder, params, actions)
            actions = actions.reshape(-1, self.skill_length,
                                      actions.shape[-1])
        ...
        reward = self.reward_calculation(rew_run, rew_ctrl, tasks)
    ...
    def reward_calculation(self, rew_run, rew_ctrl, tasks):
        ...
        reward = np.mean(-np.abs(rew_run_np-final_tasks) + 0.5 * rew_ctrl_np,
                         axis=0)
        return reward
